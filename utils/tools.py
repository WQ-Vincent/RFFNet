...
def compute_ssim(img1, img2):
    res = ssim(img2, img1, win_size=11, multichannel=True)
    return res
...
def gather_patches_into_whole(net, input_bgr, input_ir, label_bgr):
    ...
    for i, [bgr_patch, ir_patch, label_patch, bbox] in enumerate(gen_jobs(input_bgr, input_ir, label_bgr, patch_shape=(768, 512), stride=(192,128))):
        output_patch, out1_patch = net(bgr_patch, ir_patch)[:2]  #[0][:2]
...
def gather_patches_into_whole_RFF(net, input_bgr, input_ir, label_bgr):
    output_img = np.zeros(input_bgr.shape, dtype=np.float32)   # record the final fusion image
    mask = np.zeros(input_bgr.shape[-2:], dtype=np.uint8)
    for i, [bgr_patch, ir_patch, label_patch, bbox] in enumerate(gen_jobs(input_bgr, input_ir, label_bgr, patch_shape=(768, 512), stride=(192,128))):
        output_patch = net(bgr_patch, ir_patch)  #[0][:2]
        output_patch = output_patch.cpu().numpy()
...
def gather_patches_into_whole_orin(net, input_bgr, input_ir, label_bgr):
    output_img = np.zeros(input_bgr.shape, dtype=np.float32)   # record the final fusion image
    out1_img = np.zeros(input_bgr.shape, dtype=np.float32)     # record the middle denoised image
    mask = np.zeros(input_bgr.shape[-2:], dtype=np.uint8)
...
def gather_patches_into_whole_2(net, input_bgr, input_ir, label_bgr):
    output_img = np.zeros(label_bgr.shape, dtype=np.float32)   # record the final fusion image
    mask = np.zeros(label_bgr.shape[-2:], dtype=np.uint8)
    for i, [bgr_patch, ir_patch, label_patch, bbox] in enumerate(gen_jobs(input_bgr, input_ir, label_bgr, patch_shape=(800, 480), stride=(200,120))):
        output_patch = net(bgr_patch)
        output_patch = output_patch.cpu().numpy()
...
def gather_patches_into_whole_3(net, input_bgr, input_ir, label_bgr):
    output_img = np.zeros(input_bgr.shape, dtype=np.float32)   # record the final fusion image
    mask = np.zeros(input_bgr.shape[-2:], dtype=np.uint8)
    for i, [bgr_patch, ir_patch, label_patch, bbox] in enumerate(gen_jobs(input_bgr, input_ir, label_bgr, patch_shape=(800, 480), stride=(200,120))):
        output_patch = net(bgr_patch)[0]
        output_patch = output_patch.cpu().numpy()
...
def gather_patches_into_whole_CUNet(net, input_bgr, input_ir, label_bgr):
    output_img = np.zeros(input_bgr.shape, dtype=np.float32)   # record the final fusion image
    mask = np.zeros(input_bgr.shape[-2:], dtype=np.uint8)
    for i, [bgr_patch, ir_patch, label_patch, bbox] in enumerate(gen_jobs(input_bgr, input_ir, label_bgr, patch_shape=(640, 512), stride=(160,128))):
        output_patch = net(bgr_patch, ir_patch)
        output_patch = output_patch.cpu().numpy()
...
def gather_patches_into_whole_SANet(net, input_bgr, input_ir, label_bgr):
    output_img = np.zeros(input_bgr.shape, dtype=np.float32)   # record the final fusion image
    mask = np.zeros(input_bgr.shape[-2:], dtype=np.uint8)
    for i, [bgr_patch, ir_patch, label_patch, bbox] in enumerate(gen_jobs(input_bgr, input_ir, label_bgr, patch_shape=(640, 512), stride=(160,128))):
        if ir_patch.shape[1]==1:
            ir_patch = torch.cat([ir_patch,ir_patch,ir_patch], dim=1)
        o1 = net(bgr_patch[:,0,:,:].unsqueeze(1), ir_patch[:,0,:,:].unsqueeze(1))
        o2 = net(bgr_patch[:,1,:,:].unsqueeze(1), ir_patch[:,1,:,:].unsqueeze(1))
        o3 = net(bgr_patch[:,2,:,:].unsqueeze(1), ir_patch[:,2,:,:].unsqueeze(1))
        if type(o1) is tuple:
            output_patch = torch.cat([o1[0],o2[0],o3[0]],dim=1)
        else:
            output_patch = torch.cat([o1,o2,o3],dim=1)
        output_patch = output_patch.cpu().numpy()
...
def validation_on_PSNR_and_SSIM_orin(net, input_rgb, input_ir, label):
    assert type(input_rgb) is torch.Tensor
    assert type(input_ir) is torch.Tensor
    ssims_o = []
    psnrs_o = []
...
def validation_on_PSNR_and_SSIM_RFF(net, input_rgb, input_ir, label):
    assert type(input_rgb) is torch.Tensor
    assert type(input_ir) is torch.Tensor
    ssims_o = []
    psnrs_o = []
    ...
    for bgr_patch, ir_patch, label_patch, _ in gen_jobs(input_rgb, input_ir, label):
        out = torch.clamp(net(bgr_patch, ir_patch), 0, 1)
        out = out.cpu().numpy()
        inp = bgr_patch[0, ...].permute(1,2,0).cpu().numpy()
        oup = out[0, ...].transpose(1,2,0)
        lab = label_patch[0, ...].permute(1,2,0).cpu().numpy()
...
def validation_on_PSNR_and_SSIM_2(net, input_rgb, input_ir, label):
    assert type(input_rgb) is torch.Tensor
    assert type(input_ir) is torch.Tensor
    ssims_o = []
    psnrs_o = []
    ...
    for bgr_patch, ir_patch, label_patch, _ in gen_jobs(input_rgb, input_ir, label):
        out = torch.clamp(net(bgr_patch), 0, 1)
        out = out.cpu().numpy()
        inp = bgr_patch[0, ...].permute(1,2,0).cpu().numpy()
        oup = out[0, ...].transpose(1,2,0)
        lab = label_patch[0, ...].permute(1,2,0).cpu().numpy()
...
def validation_on_PSNR_and_SSIM_3(net, input_rgb, input_ir, label):
    assert type(input_rgb) is torch.Tensor
    assert type(input_ir) is torch.Tensor
    ssims_o = []
    psnrs_o = []
    ...
    for bgr_patch, ir_patch, label_patch, _ in gen_jobs(input_rgb, input_ir, label):
        out = torch.clamp(net(bgr_patch)[0], 0, 1)
        out = out.cpu().numpy()
        inp = bgr_patch[0, ...].permute(1,2,0).cpu().numpy()
        oup = out[0, ...].transpose(1,2,0)
        lab = label_patch[0, ...].permute(1,2,0).cpu().numpy()
...
def validation_on_PSNR_and_SSIM_CUNet(net, input_rgb, input_ir, label):
    assert type(input_rgb) is torch.Tensor
    assert type(input_ir) is torch.Tensor
    ssims_o = []
    psnrs_o = []
...
def validation_on_PSNR_and_SSIM_SANet(net, input_rgb, input_ir, label):
    assert type(input_rgb) is torch.Tensor
    assert type(input_ir) is torch.Tensor
    ssims_o = []
    psnrs_o = []
    ...
    for bgr_patch, ir_patch, label_patch, _ in gen_jobs(input_rgb, input_ir, label):
        o1 = net(bgr_patch[:,0,:,:].unsqueeze(1), ir_patch)
        o2 = net(bgr_patch[:,1,:,:].unsqueeze(1), ir_patch)
        o3 = net(bgr_patch[:,2,:,:].unsqueeze(1), ir_patch)
        if type(o1) is tuple:
            res = torch.cat([o1[0],o2[0],o3[0]],dim=1)
        else:
            res = torch.cat([o1,o2,o3],dim=1)
        out = torch.clamp(res, 0, 1)
        out = out.cpu().numpy()
        inp = bgr_patch[0, ...].permute(1,2,0).cpu().numpy()
        oup = out[0, ...].transpose(1,2,0)
