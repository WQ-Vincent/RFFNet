...
if not opt_train['RESUME']:
    ...
    net = load_model(opt_train['RESUME_PATH'], mode=mode, opt=opt['NETWORK'])
...
train_loader = DataLoader(dataset=train_dataset, batch_size=opt['OPTIM']['BATCH_SIZE'], shuffle=True, num_workers=24,
...
for epoch in range(start_epoch, opt_train['NUM_EPOCHS'] + 1):
    ...
    for i, data in enumerate(train_loader):
        ...
        loss_spt = criterion_recon(pred, gt_rgb) + criterion_recon(pred_mid, gt_rgb)
        ...
        if i % 50 == 0:
            outputs = [
                ...
                "loss_spt {:.4f} ".format(loss_spt.item()*1000),
    ...
    if (epoch % opt['VAL_FREQ'] == 0 and epoch > opt_train['NUM_EPOCHS']-5*opt['VAL_FREQ']) or (epoch % (opt['VAL_FREQ']*5) == 0 and epoch <= 1400):
        psnr, _ = validation(net, opt)
        if psnr > best_psnr:
            best_psnr = psnr
            best_epoch = epoch
            torch.save({'epoch': epoch,
            ...
            print('The current best model with PSNR=%.4f was trained in epoch:%d' % (best_psnr, best_epoch))
            logging.info('The current best model with PSNR=%.4f was trained in epoch:%d' % (best_psnr, best_epoch))
            continue   
        print('The current best model with PSNR=%.4f was trained in epoch:%d' % (best_psnr, best_epoch))
